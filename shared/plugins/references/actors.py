"""Actor protocol and implementations for reference selection.

Actors handle the interactive selection of reference sources when the model
...
The actor type is determined by configuration (matching the permission plugin pattern).
"""
...
class SelectionActor(ABC):
    """Base class for reference selection actors.

    Actors are responsible for presenting available references to the user
...
class ConsoleSelectionActor(SelectionActor):
    """Actor that prompts the user in the console for selection.
...
class WebhookSelectionActor(SelectionActor):
    """Actor that sends selection requests to an HTTP webhook.
    ...
    def initialize(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        """
        if not HAS_REQUESTS:
            raise RuntimeError("requests library required for WebhookSelectionActor")

        if not config:
            raise ValueError("WebhookSelectionActor requires configuration with 'endpoint'")

        self._endpoint = config.get("endpoint")
        if not self._endpoint:
            raise ValueError("WebhookSelectionActor requires 'endpoint' in config")
...
class FileSelectionActor(SelectionActor):
    ...
    def initialize(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        """
        if not config:
            raise ValueError("FileSelectionActor requires configuration with 'base_path'")

        base_path = config.get("base_path")
        if not base_path:
            raise ValueError("FileSelectionActor requires 'base_path' in config")
...
def create_actor(actor_type: str, config: Optional[Dict[str, Any]] = None) -> SelectionActor:
    ...
    Returns:
        Initialized SelectionActor instance

    Raises:
        ValueError: If actor_type is unknown
    """
    actors = {
        "console": ConsoleSelectionActor,
        "webhook": WebhookSelectionActor,
        "file": FileSelectionActor,
    }
